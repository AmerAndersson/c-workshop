...
For that, we call the stream of member function close. 
...
In this example, the entire file is read and stored in a memory block. Lets examine how this is done:
...
Explicitly, with member function sync(): Calling the streams member function sync() causes an immediate synchronization. This function returns an int value equal to -1 if the stream has no associated buffer or in case of failure. Otherwise (if the stream buffer was successfully synchronized) it returns 0.
...
#include <algorithm>
#include <iterator>
std::ifstream input("main.gif", std::ios::binary);
std::ofstream output("main.gif", std::ios::binary);
